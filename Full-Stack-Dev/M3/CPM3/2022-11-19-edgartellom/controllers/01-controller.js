...
const addCars = (car, brand) => {
  const carFound = utils.cars.find(c => c.id === car.id);
  const brandFound = utils.brands.find(b => b.name === brand.name);
  if (carFound) throw new Error ('Este coche ya existe en la base de datos');
  if (brandFound) throw new Error ('Esta marca ya existe en la base de datos');
  utils.cars.push(car);
