...
function problemB () {
  ...
  .then(stanza2 => {
    console.log('-- B. promisified version (stanza two) --');
    blue(stanza2)
  })
  promisifiedReadFile('poem-one/stanza-03.txt')
  .then(stanza3 => {
    console.log('-- B. promisified version (stanza three) --');
    blue(stanza3)
  })
...
function problemC () {
  ...
  // promisified version (hint: don't need to nest `then` calls)
  // promisifiedReadFile('poem-one/stanza-02.txt')
  // .then(stanza2 => {
